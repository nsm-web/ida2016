...
<body  style="background-image: url(images/fond.jpg);" >
  ...
  <tr>
   ...
   $requet="SELECT * FROM produit ORDER BY num_prodt ASC";
   $prepare=$connexion->prepare($requet);
   $reponse=$prepare->execute();
?>
    <table border="1">
        <tr><td rowspan="2"> Numero Produit </td>
	     <td rowspan="2"> Designation </td>
	     <td rowspan="2"> Numero Fournisseur </td>
	     <td rowspan="2"> Id Ttpe Produit </td>
		 <td colspan="2"> Action </td>
	    </tr>
	    <tr><td> Modifier </td><td> Supprimer </td></tr>
	   <?php
	    while ($donnee=$prepare->Fetch()){ ?>
			<tr><td><?php echo $donnee['num_prodt'];?></td>
			 <td><?php echo $donnee['designation'];?></td>
			 <td><?php echo $donnee['num_fourn'];?></td>
			 <td><?php echo $donnee['id_type_prodt'];?></td>
			 <td><a href="modifier_prodt.php? num=<?php echo $donnee['num_prodt'];?>"> Modifier </a></td>
			 <td><a href="suppression_prodt.php? num=<?php echo $donnee['num_prodt'];?>"> Supprimer </a></td>
			</tr>
			<?php 
	    } ?>
	</table></td>
